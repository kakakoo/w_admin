      ...
      <div class="main_container">
		...
		<div class="my_user_content">
			...
			<div class="refresh" style="width:70%;">
				...
				<input type="button" class="btn btn-dark" onclick="dataGroup()" value="지점정보 생성">
<!-- 				<input type="button" class="btn btn-success" onclick="dataExcel()" value="엑셀 받기"> -->
				<input type="button" class="btn btn-dark" onclick="openDialog()" value="예약정보 출력">
				<input type="button" class="btn btn-success" onclick="location.href='${pageContext.request.contextPath}/api/rsv/list/m'" value="간략 보기">
...
<%-- 							<option value="t" <c:if test="${paging.storeId == 't'}">selected</c:if>>전체</option> --%>
			...
			<table class="list_table" style="font-size:12px;">
				...
				<c:forEach var="items" items="${resultList }">
					...
					<tr <c:choose>
							...
							<c:when test="${items.rsvSt == 'CR' }">style="background-color:rgba(209,178,255,0.2);"</c:when>
...
<%-- 							</c:if> --%>
						...
						<td onclick="changeStore('${items.rsvId }', '${items.storeId }')"><nobr><font title="${items.storeNm }">${items.storeNm }</font></nobr></td>
...
<%-- 						<td onclick="changePaper('${items.rsvId }', '${items.rsvNm }', '${items.rsvPaper }')"> --%>
						<td>
							<c:choose>
						...
						<td style="text-align:right;"><nobr><font title="${items.basicRateWeys }"><fmt:formatNumber value="${items.basicRateWeys }" groupingUsed="true"/></font></nobr></td>
						<td style="text-align:right;"><nobr><font title="${items.weysBonus }"><fmt:formatNumber value="${items.weysBonus }" groupingUsed="true"/></font></nobr></td>
						<td style="text-align:right;"><nobr><font title="${items.getAmnt - items.commis - items.cms }"><fmt:formatNumber value="${items.getAmnt - items.commis - items.cms }" groupingUsed="true"/></font></nobr></td>
						<td style="text-align:right;"><nobr><font title="${items.cms }"><fmt:formatNumber value="${items.cms }" groupingUsed="true"/></font></nobr></td>
						<td style="text-align:right;" 
							<c:if test="${items.rsvSt == 'R'}">onclick="makeReturn('${items.rsvId }', '${items.rsvNm }')"</c:if>><nobr><font title="${items.getAmnt }"><fmt:formatNumber value="${items.getAmnt }" groupingUsed="true"/></font></nobr></td>
...
<%-- 						<td><nobr><font title="${items.adminName }">${items.adminName }</font></nobr></td> --%>
						<td><nobr><font title="${items.platform }">${items.platform }</font></nobr></td>
...
<div class="dialog" id="dialog_group">
	<div class="dialog_body" style="height:220px;margin-top:-110px;">
		<div class="dialog_title">예약 그룹 생성</div>
		<table>
			<colgroup>
				<col style="width:30%"/>
				<col style="width:70%"/>
			...
			<tr>
				<td>예약 날짜 선택</td>
				<td><input type="text" style="width:240px;display:initial;" name="rsvDt" id="grp_dt" class="form-control" value="${excelDt}" /></td>
			</tr>
		</table>
		<div class="dialog_button">
			<input type="button" class="btn btn-dark" onclick="closeDialog()" value="취소">
			<input type="button" class="btn btn-success" onclick="makePdf()" value="생성">
		</div>
	</div>
</div>
	...
	<div class="dialog_body" style="height:230px;margin-top:-115px;">
		...
		<table>
			<colgroup>
				...
				<tr>
					<td>변경일</td>
					<td><input type="text" style="width:120px;display:initial;" name="reservation" id="change_dt_dt"/> <input type="text" style="width:110px;" id="change_dt_tm"></td>
				</tr>
		</table>
		<div class="dialog_button">
			<input type="button" class="btn btn-dark" onclick="closeDialog()" value="취소">
...
<div class="dialog" id="dialog_paper">
	<div class="dialog_body" style="height:230px;margin-top:-115px;">
		<div class="dialog_title">권종 변경</div>
		...
		<table>
			<colgroup>
				...
				<tr>
					<td>고객명</td>
					<td id="change_paper_nm"></td>
				</tr>
				<tr>
					<td>권종</td>
					<td id="change_paper"></td>
				</tr>
		</table>
		<div class="dialog_button">
			<input type="button" class="btn btn-dark" onclick="closeDialog()" value="취소">
...
<div class="dialog" id="dialog_store">
	<div class="dialog_body" style="height:230px;margin-top:-115px;">
		<div class="dialog_title">예약지점 변경</div>
		...
		<table>
			<colgroup>
				...
				<tr>
					<td>고객명</td>
					<td id="change_store_nm"></td>
				</tr>
				<tr>
					<td>변경일</td>
					<td id="change_store_html"></td>
				</tr>
		</table>
		<div class="dialog_button">
			<input type="button" class="btn btn-dark" onclick="closeDialog()" value="취소">
			<input type="button" class="btn btn-success" onclick="setRsvStore()" value="변경">
		</div>
	</div>
</div>
...
<div class="dialog" id="dialog_return">
	<div class="dialog_body" style="height:400px;width:700px;margin-top:-200px;margin-left:-350px;">
		<div class="dialog_title">환불정보 등록</div>
		<table>
			<colgroup>
					<col style="width:30%"/>
					<col style="width:70%"/>
				...
				<tr>
					<td>은행</td>
					<td><input type="text" id="ret_bank_nm"></td>
				</tr>
				<tr>
					<td>계좌번호</td>
					<td><input type="text" id="ret_bank_cd"></td>
				</tr>
				<tr>
					<td>취소수수료</td>
					<td><input type="number" id="ret_cms"></td>
				</tr>
		</table>
		<input type="hidden" id="returnNm">
		<input type="hidden" id="returnRsvId">
		<div class="dialog_button">
			<input type="button" class="btn btn-dark" onclick="closeDialog()" value="취소">
			<input type="button" class="btn btn-success" onclick="updateReturn()" value="환불등록">
		</div>
	</div>
</div>
...
$(function () {
	...
	$('input[id="change_dt_dt"]').daterangepicker({
        singleDatePicker: true,
        showDropdowns: true
    },function(start, end, label) {});
	
	$('input[id="grp_dt"]').daterangepicker({
        singleDatePicker: true,
        showDropdowns: true
    },function(start, end, label) {});
...
function changeStore(rsvId, storeId){
	...
	$.ajax({
		type:"post",
		url:"${pageContext.request.contextPath}/api/rsv/getRsvStoreInfo",
		data: JSON.stringify(data),
		async : false,
		contentType: "application/json",
		success : function(result){
			
			var items = result.store;
			
			var html = '';
			html += '<select id="storeIdChg">';
			for(i=0 ; i<items.length ; i++){
				var store_id = items[i].STORE_ID;
				var store_nm = items[i].STORE_NM;
				
				if(store_id == storeId){
					html += '<option value="' + store_id + '" selected>' + store_nm + '</option>';
				} else {
					html += '<option value="' + store_id + '">' + store_nm + '</option>';
				}
			}
			html += '</selected>';
			
			$('#change_store_html').html(html);
			$('#change_store_nm').html(result.RSV_NM);
			$('#rsvId').val(rsvId);
		},
		error : function(result){
			alert('업데이트 실패, 에러');
	...
	$('.dialog_back').css('display', 'block');
	$('#dialog_store').css('display', 'block');
}

function setRsvStore(){
	
	var data = {};
	data["rsvId"] = $('#rsvId').val();
	data["storeId"] = $('#storeIdChg').val();
	
	$.ajax({
		type:"post",
		url:"${pageContext.request.contextPath}/api/rsv/updateRsvStore",
		data: JSON.stringify(data),
		async : false,
		contentType: "application/json",
		success : function(result){
...
function changePaper(rsvId, usrNm, paper){
	$('#rsvId').val(rsvId);
	$('#change_paper_nm').html(usrNm);

	var html = '<select id="paperVal">';

	if(paper == 'B'){
		html += '<option value="B" selected>고액 위주</option>';
	} else {
		html += '<option value="B">고액 위주</option>';
	}

	if(paper == 'S'){
		html += '<option value="S" selected>소액 포함</option>';
	} else {
		html += '<option value="S">소액 포함</option>';
	}

	if(paper == 'R'){
		html += '<option value="R" selected>랜덤</option>';
	} else {
		html += '<option value="R">랜덤</option>';
	}
	
	html += '</select>';
	
	$('#change_paper').html(html);
	
	$('.dialog_back').css('display', 'block');
	$('#dialog_paper').css('display', 'block');
}
...
function makePdf(){
	...
	var data = {};
	data["rsvDt"] = $('#grp_dt').val();
	
	$.ajax({
		type:"post",
		url:"${pageContext.request.contextPath}/api/rsv/makeGroup",
		data: JSON.stringify(data),
		async : false,
		contentType: "application/json",
		success : function(result){
...
function makeReturn(rsvId, rsvNm){
	$('#return_nm').html(rsvNm);
	$('#returnNm').val(rsvNm);
	$('#returnRsvId').val(rsvId);
	
	$('.dialog_back').css('display', 'block');
	$('#dialog_return').css('display', 'block');
}

function updateReturn(){
	if(confirm("환불정보를 등록하시겠습니까?")){

		var data = {};
		data["rsvId"] = $('#returnRsvId').val();
		data["rsvNm"] = $('#returnNm').val();
		data["retBankNm"] = $('#ret_bank_nm').val();
		data["retBankCd"] = $('#ret_bank_cd').val();
		data["retCms"] = $('#ret_cms').val();
		
		$.ajax({
			type:"post",
			url:"${pageContext.request.contextPath}/api/rsv/updateReturn",
			data: JSON.stringify(data),
			async : false,
			contentType: "application/json",
			success : function(result){
