    ...
    <div class="container body">
      <div class="main_container">
		...
		<div class="my_user_content">
			...
			<input type="hidden" id="usrCost" value="${info.cost }">
			<table class="list_table" style="margin-top:10px;">
				...
				<tr>
					<td>거래유형</td>
					<td style="text-align:left;">멤버십</td>
				</tr>
				<tr>
					<td rowspan="2">통화</td>
				...
				<tr>
					<td>고객 가능 환전 금액</td>
					<td style="text-align:left;" id="inputUsrMaxAmnt">${totalUsrAmnt }</td>
				</tr>
				<tr>
	...
	$.ajax({
		...
		success : function(result){
			...
			if(res == 'success'){
				...
				if(tradeType == 'S'){
					usrCost = usrCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
					$('#inputUsrMaxAmnt').html(usrCost + ' KOR'); 
				} else {
					var basicRate = $('#basicRate').val();
					var cost = 0;
					if(unitCd == 'JPY'){
						cost = Math.floor(usrCost * 100 / basicRate);
					} else {
						cost = Math.floor(usrCost / basicRate);
					}
					cost = cost.toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
					$('#inputUsrMaxAmnt').html(cost + ' ' + unitCd); 
				}
			} else {
				alert('업데이트 실패');
			}
...
function writeAction(action, barcode){
	...
	if(action == 'write'){
		...
		$.ajax({
			type:"post",
			url:"${pageContext.request.contextPath}/api/manage/checkCost",
			data: JSON.stringify(data),
			async : false,
			contentType: "application/json",
			success : function(result){
